<template>
  <div class="mt-24 min-h-screen" id="projects">
    ...
    <div class="max-w-5xl mx-auto">
      <div class="grid lg:grid-cols-3 sm:grid-cols-1 md:grid-cols-2 gap-8">
        <div class="space-y-2" v-for="project in projects" :key="project.order">
          <div class="relative group">
            <div
              class="opacity-0 z-10 group-hover:opacity-50 transition-all duration-500 ease-in-out absolute items-center justify-center w-full h-full bg-black rounded-md"
            ></div>
            <div
              class="space-y-2 opacity-0 z-20 group-hover:opacity-100 absolute transition-all duration-500 ease-in-out top-0 right-0 bottom-0 left-0 m-auto h-min w-min text-white"
            >
              <div v-if="project.github">
                <a
                  class="flex justify-center py-1 gap-2 items-center hover:underline rounded-full hover:px-2 hover:border-white transition-all duration-500 ease-in-out"
                  :href="project.github"
                  target="_blank"
                >
                  <svg
            ...
            <div
              class="relative max-w-xs overflow-hidden bg-cover bg-no-repeat rounded-md"
            >
              <img
          ...
          <div class="space-y-2">
            ...
            <ul class="flex flex-wrap justify-start gap-1">
              <li v-for="tag in project.tags" :key="tag">
                <span
                  class="px-2 py-1 text-xs text-gray-900 dark:text-gray-300 border-2 border-gray-500 rounded-full"
                  >{{ tag }}</span
                >
              </li>
            </ul>
